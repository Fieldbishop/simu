...
public class OmaMoottori extends Moottori{
	...
	protected void suoritaTapahtuma(Tapahtuma t){  // B-vaiheen tapahtumat
		...
		switch (t.getTyyppi()){
			...
			// TODO: Tässä määritellään tällä hetkellä mihin kohdetapahtumaan asiakkaat ovat menossa, ei toimi lopullisessa.
			case JONOARR:
			...
			case JONODEP:
				//erikoissääntö ekalle jonolle joka lajittelee turvatarkastukseen jos sellainen vapaana
				if(palvelupisteet[1].getJono().size() <= palvelupisteet[2].getJono().size()&& palvelupisteet[0].getJono().size() > 0) {
				...
				else if(palvelupisteet[1].getJono().size() > palvelupisteet[2].getJono().size() && palvelupisteet[0].getJono().size() > 0) {
					Asiakas palveltava = palvelupisteet[0].otaJonosta();
			...
			//Turvatarkastuksista lähteminen eli sorttaus asiakasobjektin kohdetapahtuman mukaiseen jonoon.
			case TURVADEP0:
			...
			case SALIDEP1: 
				...
				//Jos tapahtuma on loppunu ja uutta ei ole tehty, tehdään uusi tapahtuma + asiakkaat
	...
	protected void tulokset() {
		...
		for(Asiakas a : asiakasRekisteri) {
			...
			case TURVA1:
				...
				turvatarkastus1TotalPalveluAika += a.getPoistuminen(TapahtumanTyyppi.TURVADEP0)-a.getSaapumisaika(TapahtumanTyyppi.TURVADEP0);
				break;
			case TURVA2:
				turvatarkastus2Total++;
				turvatarkastus2TotalPalveluAika += a.getPoistuminen(TapahtumanTyyppi.TURVADEP1)-a.getSaapumisaika(TapahtumanTyyppi.TURVADEP1);
				break;
			default:
				break;
			}
			...
			case RESPADEP:
				respaTotal++;
				respaTotalPalveluAika += a.getPoistuminen(TapahtumanTyyppi.RESPADEP)-a.getSaapumisaika(TapahtumanTyyppi.RESPADEP);
				break;
			case SALIDEP1:
				sali1Total++;
		...
		System.out.println("Total systeemissä asioineet asiakkaat: " + asiakasRekisteri.size() +
				...
				"\ntt2: " + turvatarkastus2TotalPalveluAika +
				"\nrespa: " + respaTotalPalveluAika +
				"\nsali1: " + sali1TotalPalveluAika +
		...
		switch(asiakas.getKohdeTapahtuma()) {
			...
			case SALIDEP2:
				if(!salit[1].onAlkanut() && !salit[1].onTäynnä()) {
					...
					kontrolleri.setSaliYleisöTeksti(Paikka.SALI2, salit[1].getSalissaOlleetMäärä() + "/" +  salit[1].getYleisöMaksimi());
				}
				else {
					Trace.out(Trace.Level.INFO, "Asiakas " + asiakas.getId() + " ei päässyt istuntoon");
					asiakas.setPoistumisaika2(Kello.getInstance().getAika());
